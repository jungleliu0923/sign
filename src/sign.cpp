...
/* 
 * @date : 2014/01/04 20时45分54秒
 * @author : liujun (liujun0923@zju.edu.cn)
 * @brief : 根据字符串生成crc64校验数据 
 * @input  : 校验数据
 * @return : unsign long long 数据
 */
...
/* 
 * @date : 2014/01/04 21时09分10秒
 * @author : liujun (liujun0923@zju.edu.cn)
 * @brief : 是否是数字 
 * @input  :  字符串str, 数字返回digit
 * @return : 是否是数字以及数字内容digit
 */
...
/* -----  end of function is_digit  ----- */


/* 
 * @date : 2014/01/04 18时50分20秒
 * @author : liujun (liujun0923@zju.edu.cn)
 * @brief : 对字符串进行签名, sign1是签名高位，sign2是签名低位 
 * @input  :  input, sign1, sign2
 * @return : 0成功，-1失败
 */
int create_sign64 (const char* input, unsigned int* sign1, unsigned int* sign2)
{
    int ret;
    ret = is_digit(input, sign2);
    //如果是数字, sign2返回，以及
    if(ret == 0)
    {
    	*sign1 = 0;
    ...
    unsigned long long crc_res = crc64((unsigned const char*)input);
    crc_sign64* crc_sign_res = (crc_sign64*)&crc_res;
    //签名高位
    *sign1 = crc_sign_res->sign1;
    //签名低位
    *sign2 = crc_sign_res->sign2;
    return 0;
}
/* -----  end of function create_sign64  ----- */
